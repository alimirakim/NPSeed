...
import { ViewOdds } from './TraitsOfType'
...
export default function TraitFieldB({ traitType, traitsOfType, tagTypeIds }) {
  const dispatch = useDispatch()
  const [value, setValue] = useState({ trait: "", tagIds: [] })

  useEffect(() => {
    if (value && value.id) {
      dispatch(updateGenSetting({ traitType, trait: value.id }))
    } else if (value) {
      dispatch(updateGenSetting({ traitType, trait: value.trait }))
    } else {
      dispatch(updateGenSetting({ traitType, trait: "" }))
  ...
  return (
    <>
      <Autocomplete
        ...
        handleHomeEndKeys
        freeSolo
        renderInput={(params) => <TextField {...params} label={traitType} />}
      />
